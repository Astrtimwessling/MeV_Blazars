...
def k_correction_mono(nuLnu, z, E):
...
newASTROGAM_Sensitivity_Table = Table.read(config['Catalog_dir'] + 'newASTROGAM_sensitivity.csv', format='ascii.no_header', names=['Energy', 'Sensitivity'])
AMEGO_X_Sensitivity_Table = Table.read(config['Catalog_dir'] + 'AMEGO_X_Sensitivity_Points.csv', format='ascii.no_header', names=['Energy', 'Sensitivity'])
...
GRAMS_Balloon_Sensitivity = Sensitivity_Data['_GRAMS_Balloon_y'].tolist()[:9]
GRAMS_Satellite_Sensitivity = Sensitivity_Data['_GRAMS_Satellite_y'].tolist()[:9]

newASTROGAM_sensitivity = newASTROGAM_Sensitivity_Table['Sensitivity'].tolist()
AMEGO_X_sensitivity = AMEGO_X_Sensitivity_Table['Sensitivity'].tolist()

z_space = np.linspace(0, 8, 100)
    ...
    for f in range(len(GRAMS_Flux_Bins)):
        for s in range(len(z_space)):
            ...
            def dN_dz(z, F_lim, E):
                ...
                kcorr = k_correction_mono(nuLnu_SBPL, z, E)

                log_L_min = np.log10(4*np.pi* (d_L*3.085677581e24)**2* F_lim* kcorr)

                def integrand(log_L):
                    L = 10**log_L
                    return L*LDDE_BLL1(L, z)
            ...
            N_z_GRAMS_Balloon = quad(dN_dz, z_space[s], 8, args=(GRAMS_Balloon_Sensitivity[f], GRAMS_Flux_Bins[f]))[0] * 4 * np.pi
            N_z_GRAMS_Satellite = quad(dN_dz, z_space[s], 8, args=(GRAMS_Satellite_Sensitivity[f], GRAMS_Flux_Bins[f]))[0] * 4 * np.pi
            N_z_newASTROGAM = quad(dN_dz, z_space[s], 8, args=(newASTROGAM_sensitivity[f], GRAMS_Flux_Bins[f]))[0] * 4 * np.pi
            N_z_AMEGO_X = quad(dN_dz, z_space[s], 8, args=(AMEGO_X_sensitivity[f], GRAMS_Flux_Bins[f]))[0] * 4 * np.pi
            
            print(N_z_GRAMS_Balloon, N_z_GRAMS_Satellite, N_z_newASTROGAM, N_z_AMEGO_X)
...
for i in range(len(BLL_LP_Catalog)):
    ...
    for f in range(len(GRAMS_Reduced_Flux_Bins)-1):
        K_correction_band = k_correction_mono(nuLnu_LP, z, GRAMS_Flux_Bins[f])
...
for s in range(len(N_S_bins_LDDE1)):
    ...
    plt.xlabel(f'Sensitivity ({GRAMS_Flux_Bins[s]} MeV)' r'(erg cm$^{-2}$ s$^{-1}$)')
    plt.ylabel(r'N(>S) [4$\pi$ str]')
    plt.xscale('log')
    plt.yscale('log')
    #plt.title(f'N(>S) {GRAMS_Reduced_Flux_Bins}')
    plt.legend()
    plt.savefig(out_dir + f'N_S_{GRAMS_Flux_Bins[s]}_MeV.png',bbox_inches='tight',dpi=300)
    plt.show()
    plt.close()
    
LF_Data_Table = Table([F_lim, N_S_bins_LDDE1[0], N_S_bins_LDDE1[1], N_S_bins_LDDE1[2], N_S_bins_LDDE1[3], N_S_bins_LDDE1[4], N_S_bins_LDDE1[5], N_S_bins_LDDE1[6], N_S_bins_LDDE1[7], N_S_bins_LDDE1[8], N_S_bins_LDDE2[0], N_S_bins_LDDE2[1], N_S_bins_LDDE2[2], N_S_bins_LDDE2[3], N_S_bins_LDDE2[4], N_S_bins_LDDE2[5], N_S_bins_LDDE2[6], N_S_bins_LDDE2[7], N_S_bins_LDDE2[8], N_S_bins_LDDE_BLL1_all[0], N_S_bins_LDDE_BLL1_all[1], N_S_bins_LDDE_BLL1_all[2], N_S_bins_LDDE_BLL1_all[3], N_S_bins_LDDE_BLL1_all[4], N_S_bins_LDDE_BLL1_all[5], N_S_bins_LDDE_BLL1_all[6], N_S_bins_LDDE_BLL1_all[7], N_S_bins_LDDE_BLL1_all[8]], names=['Sensitivity', 'Toda_0_2', 'Toda_0_5','Toda_1_0', 'Toda_2_0', 'Toda_5_0', 'Toda_10_0', 'Toda_20_0', 'Toda_50_0', 'Toda_100_0', 'Rajguru_0_2', 'Rajguru_0_5', 'Rajguru_1_0', 'Rajguru_2_0', 'Rajguru_5_0', 'Rajguru_10_0','Rajguru_20_0', 'Rajguru_50_0', 'Rajguru_100_0', 'Ajello_BLL_0_2', 'Ajello_BLL_0_5', 'Ajello_BLL_1_0', 'Ajello_BLL_2_0', 'Ajello_BLL_5_0', 'Ajello_BLL_10_0', 'Ajello_BLL_20_0', 'Ajello_BLL_50_0', 'Ajello_BLL_100_0'])
LF_Data_Table.write(config['Catalog_dir'] + 'Monochromatic_Luminosity_Function_Results.fits', overwrite=True)
