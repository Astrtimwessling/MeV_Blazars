...
for i, ax in enumerate(axes):
    ...
    for b_deg in b_deg_list:
        ax.text(0.0, np.deg2rad(b_deg) + 0.05, f"{b_deg}°", color='black', ha='right', va='center', fontsize=9)

    l_deg_list = [-150, -120, -90, -60, -30, 0, 30, 60, 90, 120, 150]

    for l_deg in l_deg_list:
        ax.text(-np.deg2rad(l_deg) - 0.1, 0.0, f"{l_deg}°", color='black', ha='center', va='bottom', fontsize=9)

    ax.set_title(f"{MeV_Energies[i]} MeV", color='black', fontsize=18, pad=12)
...
cbar.set_label(r"Energy Flux [erg cm$^{-2}$ s$^{-1}$]", fontsize=20)

fig.legend(handles=legend_elements, loc='lower center', bbox_to_anchor=(0.51, 0.16), ncol=3, fontsize=18, frameon=False, handletextpad=0.01, columnspacing=1.5)
...
for i in range(len(MeV_Flux_Estimates_By_Band)):
    ...
    for x in range(len(RA_Sources)):
        ...
        b = coords_gal.b.radian
        norm = LogNorm(vmin=1e-12, vmax=0.3e-9)
        sc = ax.scatter(l, b, c=Flux, cmap='inferno', norm=norm, s=30, marker = Blazar_Marker[x], label=Blazar_Label[x])
    ...
    ax.set_yticklabels([])
    ax.set_xticklabels([])
    ax.legend(handles=legend_elements, loc='lower center', bbox_to_anchor=(0.9, -0.3), ncol=1, fontsize=15, frameon=False, handletextpad=0.01, columnspacing=1.5)
