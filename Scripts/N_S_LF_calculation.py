...
from src.LDDE_Luminosity_Functions import LDDE1, LDDE2, LDDE3, LDDE_BLL1, N_S_integrand
...
for i in range(len(BLL_LP_Catalog)):
    ...
    for f in range(len(GRAMS_Flux_Bins)):  
        for s in range(len(F_lim)):
            ...
            N_S_integrand_bll = N_S_integrand(nuLnu=nuLnu_LP_bin, LDDE=LDDE_BLL1, Energy=GRAMS_Flux_Bins[f], z_min=0.03, z_max=6, F_lim=F_lim_band, Lum_bin=Lum_bin, log_L_max=log_L_max, i=i)
            N_LDDE_BLL1 = quad(N_S_integrand_bll, 0.03, 6, epsabs=1e-6, epsrel=1e-4)[0] * 4 * np.pi
